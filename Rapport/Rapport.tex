...
\begin{itemize}
    ...
    \item \(\mathcal{F}_t\) : filtration représentant l’ensemble de l’information disponible jusqu’à la date \(t\).
\end{itemize}
...
\]
la démonstration du passage de l’équation différentielle stochastique à cette expression explicite étant détaillée en annexe~A.
...
\right\}
\)
, c'est-à-dire l’ensemble des temps d’arrêt \(\tau\) tels que \(\tau \ge t_n\). Cette formulation met en évidence que le prix de l’option américaine correspond à la meilleure espérance actualisée que l’on puisse obtenir en choisissant de manière optimale la date d’exercice.
...
) qui n’admet pas, en général, de forme fermée. L’algorithme de Longstaff--Schwartz répond à cette difficulté en approximant la valeur de continuation par régression sur une base de fonctions de \(S_{t_n}\) à partir de trajectoires simulées, permettant ainsi d’implémenter la règle d’exercice précédente par induction arrière.
...
\emph{induction arrière}, en partant de la maturité et en remontant jusqu’à la
date initiale.
...
\begin{itemize}
    ...
    \]
    \item seules les trajectoires \emph{in-the-money} à la date $t_n$ sont
...
course. Le schéma \texttt{schedule(static)} est privilégié afin d’assurer une
...
massif offert par les architectures CUDA. Le GPU est particulièrement adapté à
...
de garantir des accès coalescents et de limiter les transferts entre l’hôte
...
Afin de valider nos résultats Monte Carlo et de disposer d'un point de comparaison déterministe performant, nous avons également implémenté des solveurs basés sur les différences finies (FDM) pour l'équation de Black-Scholes-Merton (PDE). Bien que ces méthodes soient limitées en dimension (difficiles à étendre au-delà de 2 ou 3 sous-jacents), elles sont extrêmement efficaces pour les options vanilles et américaines sur un seul sous-jacent.
...
\begin{itemize}
    ...
    \item \textbf{Euler Implicite} : Schéma inconditionnellement stable, nécessitant la résolution d'un système linéaire tridiagonal à chaque pas de temps (algorithme de Thomas).
    \item \textbf{Runge-Kutta 4 (RK4)} : Méthode d'ordre 4 en temps, offrant une très grande précision mais plus coûteuse en calculs par itération.
\end{itemize}
...
\hline
\end{tabular}
\caption{Comparaison des temps de calcul et précision pour un Put Américain ($S_0=100, K=100, r=0.05, \sigma=0.2, T=1$).}
\label{tab:results}
\end{table}
...
\subsection{Choix et stabilité de la régression}
...
surcoût de calcul.  
Dans ce cadre, une étude comparative a été menée entre la base canonique $(1, S, S^2)$ et la base de \textbf{polynômes d'Hermite de degré 2} $(1, S, S^2-1)$. Les benchmarks montrent que pour ce problème spécifique, le choix de la base n'impacte pas significativement la précision ou le temps de calcul sur GPU. Toutefois, la base d'Hermite a été retenue par défaut pour ses propriétés théoriques de meilleure stabilité numérique (conditionnement de la matrice $A^T A$).

\subsection{Extension : Bases de Régression Avancées}
Au-delà des bases quadratiques, nous avons implémenté et testé trois bases supplémentaires pour capturer plus finement la valeur de continuation :
\begin{itemize}
    \item \textbf{Laguerre} : Polynômes orthogonaux sur $[0, \infty[$, naturellement adaptés aux prix d'actifs positifs.
    \item \textbf{Tchebychev} : Polynômes minimisant l'erreur d'interpolation sur $[-1, 1]$. Une étape de normalisation (Min-Max) a été intégrée, nécessitant un kernel de réduction dédié sur GPU.
    \item \textbf{Cubique} : Base monômiale enrichie $(1, S, S^2, S^3)$.
\end{itemize}

\textbf{Analyse des résultats :}
Les benchmarks (voir \texttt{benchmark\_boosted.csv}) indiquent que la base \textbf{Cubique} permet d'atteindre un prix de $\approx 6.06$, plus proche de la référence théorique ($\approx 6.08$) que les bases quadratiques ($\approx 5.58$ dans nos tests "boosted"). Cela confirme l'intérêt d'augmenter le degré de la base pour ce type d'option, malgré le surcoût lié à la résolution de systèmes linéaires $4 \times 4$. L'implémentation GPU via un solveur de Gauss générique permet de traiter ces bases efficacement sans impact majeur sur le temps total d'exécution.
...
L’implémentation GPU avec CUDA a mis en évidence un contraste marqué entre les
...
\begin{itemize}
    ...
    \item des bases adaptatives dépendant de la distribution du sous-jacent.
\end{itemize}
...
\begin{itemize}
    \item des méthodes par arbres (binomiaux, trinômiaux) ;
...
Ce projet a été réalisé en binôme par \textbf{Florian Barbe} et \textbf{Narjisse}. Afin d'optimiser notre efficacité, nous nous sommes réparti les tâches selon nos affinités et compétences techniques :
...
D’un point de vue algorithmique et logiciel, une implémentation complète de la méthode a été réalisée en \texttt{C++}. La version séquentielle a servi de base de validation fonctionnelle, tandis que la parallélisation sur CPU via OpenMP a permis d’exploiter efficacement le parallélisme multi-cœurs. Cette approche apporte des gains de performance significatifs, bien que limités par la bande passante mémoire et la nature statistique de l’algorithme.
...
De manière générale, ce projet illustre clairement l’importance du choix de l’architecture de calcul pour les algorithmes numériques intensifs. Il montre également que les performances ne dépendent pas uniquement de la puissance brute du matériel, mais aussi de l’adéquation entre la structure de l’algorithme et le modèle de parallélisme sous-jacent.
...
\section*{Annexe A — Résolution de l’équation différentielle stochastique du mouvement brownien géométrique sous la mesure risque-neutre}
