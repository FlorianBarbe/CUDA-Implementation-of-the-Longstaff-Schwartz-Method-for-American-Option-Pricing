...
\subsection{Problème d'arrêt optimal et dynamique}
...
\[ V_0 = \sup_{\tau \in \mathcal{T}_{0,T}} \mathbb{E}^{\mathbb{Q}}\left[ e^{-r\tau} (K - S_\tau)^+ \right] \]
où le sous-jacent $(S_t)_{t\ge0}$ suit un Mouvement Brownien Géométrique (GBM) régi par l'EDS :
...
\section{Compléments de Performance OpenMP}
\label{appendix:openmp}
...
\paragraph{Cadre probabiliste}
\begin{itemize}
    \item $(\Omega, \mathcal{F}, (\mathcal{F}_t)_{t \ge 0}, \mathbb{P})$ : espace probabilisé filtré usuel.
    \item $\mathbb{Q}$ : mesure risque-neutre.
    \item $W_t^{\mathbb{Q}}$ : mouvement brownien standard sous $\mathbb{Q}$.
\end{itemize}

\paragraph{Modèle de marché}
\begin{itemize}
    \item $S_t$ : prix du sous-jacent à $t$.
    \item $r, \sigma$ : taux sans risque et volatilité (constants).
    \item $T, K$ : maturité et strike de l'option.
    \item $\Phi(S)$ : payoff ($\max(K-S, 0)$ pour un Put).
\end{itemize}

\paragraph{Paramètres numériques}
\begin{itemize}
    \item $N_{\text{steps}}, \Delta t$ : nombre de pas de temps et pas de temps.
    \item $N_{\text{paths}}$ : nombre de simulations Monte Carlo.
    \item $V_{t_n}$ : valeur de l'option à la date discrète $t_n$.
    \item $\widehat{C}(t, S)$ : estimateur de la valeur de continuation par régression.
    \item $\psi_k(S)$ : fonctions de base pour la régression LSMC.
\end{itemize}
