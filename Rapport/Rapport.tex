...
Pour valoriser l'option, nous modélisons la dynamique du sous-jacent $S_t$ sous la mesure risque-neutre $\mathbb{Q}$ par un Mouvement Brownien Géométrique (GBM). Ce modèle standard est régi par l'équation différentielle stochastique (EDS) suivante :
\[
\mathrm{d}S_t = r S_t\,\mathrm{d}t + \sigma S_t\,\mathrm{d}W_t^{\mathbb{Q}},
\]
où $r$ est le taux sans risque, $\sigma$ la volatilité constante, et $W_t^{\mathbb{Q}}$ un mouvement brownien standard (voir Annexe~\ref{appendix:brownian} pour les définitions et propriétés élémentaires).

La solution explicite de cette EDS, dont la dérivation détaillée est fournie en Annexe~A, est donnée par :
\[
S_t = S_0 \exp\left( \left(r - \frac{\sigma^2}{2}\right)t + \sigma W_t^{\mathbb{Q}} \right).
\]

Pour la simulation numérique, nous discrétisons cette dynamique sur une grille temporelle $t_0, \dots, t_N$ avec un pas $\Delta t$. La relation de récurrence utilisée pour générer les trajectoires est exacte :
\[
S_{t_{n+1}} = S_{t_n} \exp\left( \left(r - \frac{\sigma^2}{2}\right)\Delta t + \sigma \sqrt{\Delta t} Z_n \right),
\]
avec $Z_n \sim \mathcal{N}(0,1)$ i.i.d.
...
\subsection{Synthèse algorithmique}

L’algorithme de Longstaff--Schwartz repose sur l'estimation de l'espérance conditionnelle $\mathbb{E}^{\mathbb{Q}}[e^{-r\Delta t} V_{t_{n+1}} | S_{t_n}]$ par projection sur un espace vectoriel de dimension finie, engendré par une base de fonctions $(\psi_k)_{k=1..K}$.

Concrètement, la procédure par \emph{backward induction} se déroule comme suit :
\begin{enumerate}
    \item \textbf{Initialisation} : Calcul des payoffs $V_{t_N}^{(i)} = \Phi(S_{t_N}^{(i)})$ à maturité pour chaque trajectoire.
    \item \textbf{Itération} ($n = N-1 \to 1$) :
    \begin{itemize}
        \item Sélection des trajectoires \emph{in-the-money} (ITM) : seules celles où l'exercice est pertinent contribuent à la régression.
        \item Régression moindres carrés : on projette les cashflows actualisés $Y^{(i)} = e^{-r\Delta t} V_{t_{n+1}}^{(i)}$ sur la base de fonctions évaluée en $S_{t_n}$. Cela fournit l'estimateur $\widehat{C}(t_n, S) = \sum \beta_k \psi_k(S)$.
        \item Décision optimale : pour chaque chemin, on exerce si $\Phi(S_{t_n}) > \widehat{C}(t_n, S_{t_n})$.
    \end{itemize}
    \item \textbf{Finalisation} : La valeur de l'option en $t=0$ est la moyenne des flux de trésorerie actualisés selon la stratégie optimale déterminée.
\end{enumerate}
...
\textbf{Synthèse :} Le GPU excelle sur le parallélisme de données (trajectoires), offrant des speedups d'un ordre de grandeur, mais l'accélération globale reste structurellement bornée par la nature séquentielle temporelle de l'algorithme (loi d'Amdahl appliquée à la backward induction).
