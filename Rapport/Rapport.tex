...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/gpu_memory_architecture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/ada_sm_architecture.png}
...
\subsubsection{Analyse de la convergence (Figure 14)}

Les résultats présentés en figure 14 montrent l'évolution de l'erreur relative par rapport au prix RK4 de référence. Cette dynamique peut être mise en perspective avec la Figure 15 (issue de \cite{Benguigui}) qui illustre les attentes théoriques de convergence. Contrairement à l'intuition théorique qui suggère une amélioration monotone, les courbes observées ne présentent pas une décroissance régulière. Plusieurs facteurs peuvent expliquer ce comportement mitigé et "non concluant" sur ce cas test spécifique :
...
\subsection{Impact de la taille des blocs GPU}
\label{sec:gpu_block_benchmark}

Comme mentionné en Section 3.4, la taille des blocs de threads est un paramètre configurable en CUDA. Nous avons donc réalisé une série de benchmarks pour évaluer l'impact de ce paramètre sur les performances de notre implémentation LSMC.
...
Les résultats montrent une différence notable entre la plus petite taille (32 threads, soit un seul warp) et les tailles supérieures. Au-delà de 64 threads par bloc, les performances sont relativement stables. Cette observation suggère que :
\begin{itemize}
    \item Un bloc de 32 threads sous-utilise les ressources du SM (trop peu de warps actifs pour masquer les latences mémoire) ;
    \item Les tailles de 64 à 512 threads offrent des performances similaires, indiquant que l'occupancy atteint un niveau suffisant ;
    \item La taille de 256 threads (valeur par défaut dans notre implémentation) représente un bon compromis.
\end{itemize}
...
Enfin, ce projet ouvre la voie à des comparaisons plus larges entre différentes approches numériques pour le pricing d’options américaines. Comme illustré par la Figure \ref{fig:methods_comparison}, il serait intéressant de confronter les performances du LSMC à :
\begin{itemize}
