/**
 * @file gbm.cpp
 * @brief Simulation CPU du Mouvement Brownien Géométrique (GBM)
 *
 * Ce fichier contient l'implémentation CPU (séquentielle et OpenMP)
 * de la simulation Monte Carlo selon un processus GBM.
 *
 * Fonctionnalités :
 * - Simulation d'une trajectoire unique
 * - Simulation parallèle de N trajectoires (OpenMP)
 * - Export CSV des trajectoires pour visualisation
 *
 * La discrétisation utilise la solution exacte du GBM :
...
#pragma omp parallel for schedule(static)
  ...
  for (int i = 0; i < N_paths; ++i) {
    ...
    double *path_ptr =
        paths + static_cast<size_t>(i) * (static_cast<size_t>(N_steps) + 1);
